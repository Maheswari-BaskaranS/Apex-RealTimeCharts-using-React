...
const RealTimeLineCharts = () => {
  ...
  // const deleteSeries = (id) => {
  //   const updatedSeries = series.filter((s) => s.id !== id);
  //   console.log("Updated Series after deletion:", updatedSeries);
  
  //   // Update IDs of the remaining series
  //   const updatedSeriesWithNewIDs = updatedSeries.map((s, index) => ({
  //     ...s,
  //     id: index + 1,
  //   }));
  //   console.log("Updated Series with new IDs:", updatedSeriesWithNewIDs);
  
  //   setSeries(updatedSeriesWithNewIDs);
  // };
  const deleteSeries = (seriesToDelete) => {
    const updatedSeries = series.filter((s) => s !== seriesToDelete);
    setSeries(updatedSeries);
  };
  ...
  <div key={s.id}>
    <input
      ...
      onChange={(e) => handleColorChange(s.id, e.target.value)}
    />
    <button onClick={() => deleteSeries(s)}>Delete Series {s.name}</button>
  </div>
))}
